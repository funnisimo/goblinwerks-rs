...
///    // ...
/// }
/// # gw_ecs::system::assert_is_system(my_system);
/// ```
///
/// # Implementing
///
...
impl<'w, 's> Commands<'w, 's> {
    ...
    /// }
    /// # gw_ecs::system::assert_is_system(example_system);
    /// ```
    ///
    /// # See also
    ///
    ...
    /// - [`spawn_batch`](Self::spawn_batch) to spawn entities with a bundle each.
    pub fn spawn<'a, T: ComponentSet>(&'a mut self, bundle: T) -> EntityCommands<'w, 's, 'a> {
    ...
    pub fn get_entity<'a>(&'a mut self, entity: Entity) -> Option<EntityCommands<'w, 's, 'a>> {
        self.entities.is_alive(entity).then_some(EntityCommands {
            entity,
            commands: self,
        })
    }
    ...
    // /// ]);
    // /// # }
    // /// # gw_ecs::system::assert_is_system(system);
    // /// ```
    // ///
    // /// # See also
    // ///
    ...
    /// # }
    /// # gw_ecs::system::assert_is_system(initialise_scoreboard);
    /// ```
    ...
    ///
    /// ```
    /// # use gw_ecs::{system::Command, prelude::*};
    /// #[derive(Resource, Default)]
    /// struct Counter(u64);
    ///
    /// struct AddToCounter(u64);
    ...
    /// # gw_ecs::system::assert_is_system(add_three_to_counter_system);
    /// # gw_ecs::system::assert_is_system(add_twenty_five_to_counter_system);
    /// ```
...
/// # use gw_ecs::prelude::*;
/// use gw_ecs::system::EntityCommand;
/// #
...
impl<'w, 's, 'a> EntityCommands<'w, 's, 'a> {
    ...
    ///         ));
    /// }
    /// # gw_ecs::system::assert_is_system(add_combat_stats_system);
    /// ```
    pub fn insert<C: ComponentSet>(&mut self, bundle: C) -> &mut Self {
    ...
    /// }
    /// # gw_ecs::system::assert_is_system(remove_combat_stats_system);
    /// ```
    pub fn remove<T: ComponentSet>(&mut self) -> &mut Self {
    ...
    /// }
    /// # gw_ecs::system::assert_is_system(remove_character_system);
    /// ```
    ...
    T: ComponentSet + 'static,
{
    fn write(self, world: &mut World) {
        ...
        self.bundle.insert(world, entity);
    }
}
...
pub struct InitResource<R: Resource + Send + Sync + FromWorld> {
    _phantom: PhantomData<R>,
}

impl<R: Resource + Send + Sync + FromWorld> Command for InitResource<R> {
    fn write(self, world: &mut World) {
        world.ensure_resource::<R>();
    }
}

pub struct InsertResource<R: Resource + Send + Sync> {
    pub resource: R,
}
...
unsafe impl<R: Resource> Send for RemoveResource<R> {}
unsafe impl<R: Resource> Sync for RemoveResource<R> {}
...
mod tests {
    ...
    fn commands_generic() {
        ...
        world.register::<W<u32>>();
        world.register::<W<u64>>();
        ...
        assert!(world.entities().join().count() == 1);

        println!("Added entity");

        {
            let w32 = world.read_component::<W<u32>>();

            println!("w32");
            w32.join().for_each(|v| println!("{:?}", v));

            let w64 = world.read_component::<W<u64>>();

            println!("w64");
            w64.join().for_each(|v| println!("{:?}", v));
        ...
        // test adding simple (FnOnce) commands
        {
            ...
            commands.add(|world: &mut World| {
                ...
                world.spawn((W(23u32), W(16u64)));
                println!("after spawn command");
            });
...

        {
            ...
            let results3 = (&w32, &w64)
                .join()
                .map(|(a, b)| (a.0, b.0))
                .collect::<Vec<_>>();
            assert_eq!(results3, vec![(23u32, 16u64), (0u32, 42u64)]);
        }
    ...
    fn remove_components() {
        ...
        {
            let w64 = world.read_component::<W<u64>>();
            let results = w64.join().map(|a| a.0).collect::<Vec<_>>();
            assert_eq!(results, vec![]);
        }
    ...
    fn remove_resources() {
        ...
        assert!(world.has_resource::<W<i32>>());
        assert!(world.has_resource::<W<f64>>());
