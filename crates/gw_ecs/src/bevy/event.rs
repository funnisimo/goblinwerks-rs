...
use crate::legion::ResMut;
use crate::{self as gw_ecs, ReadRes};
use gw_macro::SystemParam;
...
/// # Usage
///
/// `EventWriter`s are usually declared as a [`SystemData`].
/// ```
/// # use bevy_ecs::prelude::*;
///
...
fn event_trace<E: Event>(id: EventId<E>) {
    println!("TRACE - EventReader::iter() -> {}", id);
}
...
impl<E: Event> Events<E> {
    ...
    pub fn send(&mut self, event: E) {
        ...
        println!("TRACE - Events::send() -> id: {}", event_id);
...
mod tests {
    use super::*;
    use crate::{bevy::SystemState, schedule::Schedule, World};
    ...
    //     schedule.add_system("UPDATE", |mut events: EventReader<TestEvent>| {
