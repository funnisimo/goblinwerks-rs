...
use crate::{legion::ResMut, world::FromWorld, Entities, ReadRes, Resource, World};
use std::{
    borrow::Cow,
    fmt::Debug,
    // marker::PhantomData,
...
// unsafe impl<'a, T: Resource> SystemParam for ReadRes<'a, T> {
//     type State = ();
//     type Item<'w, 's> = ReadRes<'w, T>;
...
// unsafe impl<'a, T: Resource> ReadOnlySystemParam for Option<ReadRes<'a, T>> {}

// SAFETY: this impl defers to `Res`, which initializes and validates the correct world access.
unsafe impl<'a, T: Resource> SystemParam for Option<ReadRes<'a, T>> {
    type State = ();
    type Item<'w, 's> = Option<ReadRes<'w, T>>;
    ...
    unsafe fn get_param<'w, 's>(
        ...
        world.try_read_resource::<T>().map(|v| v.into())
    }
}
...
//     #[derive(SystemParam)]
//     pub struct TupleParam<'w, 's, R: Resource, L: FromWorld + 'static>(Res<'w, R>, Local<'s, L>);
