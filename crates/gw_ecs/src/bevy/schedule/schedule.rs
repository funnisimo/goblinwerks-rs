...
pub struct ScheduleGraph {
    ...
    conflicting_systems: Vec<(NodeId, NodeId, Vec<ResourceId>)>,
    changed: bool,
...
impl ScheduleGraph {
    ...
    ///
    /// If the `Vec<ResourceId>` is empty, the systems conflict on [`World`] access.
    /// Must be called after [`ScheduleGraph::build_schedule`] to be non-empty.
    pub fn conflicting_systems(&self) -> &[(NodeId, NodeId, Vec<ResourceId>)] {
...
impl ScheduleGraph {
    ...
    fn contains_conflicts(&self, conflicts: &[(NodeId, NodeId, Vec<ResourceId>)]) -> bool {
    ...
    fn report_conflicts(
        &self,
        ambiguities: &[(NodeId, NodeId, Vec<ResourceId>)],
        components: &Components,
    ) {
