use std::{collections::HashSet, marker::PhantomData, ops::Deref};
    ...
    T: System<'a>,
{
    ...
    fn setup(&mut self, world: &mut World) {
        <T as System<'a>>::setup(self, world);
    }

    fn teardown(self: Box<Self>, world: &mut World) {
        <T as System<'a>>::teardown(*self, world);
    }

    fn reads(&self) -> HashSet<ResourceId> {
        <T as System<'a>>::reads(self)
    }

    fn writes(&self) -> HashSet<ResourceId> {
        <T as System<'a>>::writes(self)
    }
}
...
pub trait System<'a> {
    ...
    /// This commonly removes components from `world` which depend on external
    /// resources.
    fn teardown(self, world: &mut World)
    where
        Self: Sized,
    {
        let _ = world;
        ...
        impl<'a, $($ty),*> SystemData<'a> for ( $( $ty , )* )
            where $( $ty : SystemData<'a> ),*
            {
                ...
                fn reads() -> HashSet<ResourceId> {
                    ...
                    $( {
                        for i in <$ty as SystemData>::reads().into_iter() {
                            r.insert(i);
                        }
                    } )*
                ...
                fn writes() -> HashSet<ResourceId> {
                    ...
                    $( {
                        for i in <$ty as SystemData>::writes().into_iter() {
                            r.insert(i);
                        }
                    } )*
