...
unsafe impl<'a, T: Resource + Send + Sync> SystemParam for ResMut<'a, T> {
    ...
    fn init_state(_world: &mut World, system_meta: &mut SystemMeta) -> Self::State {
        ...
        if combined_access.has_write(&item) {
            ...
            panic!(
                "error[B0002]: ResMut<{}> in system {} conflicts with a previous ResRef<{0}> access. Consider removing the duplicate access.",
                std::any::type_name::<T>(), system_meta.name);
        }
...
// pub struct ResRef<'a, T: Resource + Send + Sync> {
//     fetch: ResRef<'a, T>,
// }
...
// impl<'w, T> Debug for ResRef<'w, T>
// where
//     T: Debug + Resource + Send + Sync,
// {
//     fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
//         f.debug_tuple("ResRef")
...
// {
//     fn clone(&self) -> Self {
//         ResRef::new(ResRef::clone(&self.fetch))
//     }
// }
...
unsafe impl<'a, T: Resource + Send + Sync> ReadOnlySystemParam for Option<ResRef<'a, T>> {}

// SAFETY: this impl defers to `Res`, which initializes and validates the correct world access.
unsafe impl<'a, T: Resource + Send + Sync> SystemParam for Option<ResRef<'a, T>> {
    type State = ();
    type Item<'w, 's> = Option<ResRef<'w, T>>;

    fn init_state(world: &mut World, system_meta: &mut SystemMeta) -> Self::State {
        ResRef::<'a, T>::init_state(world, system_meta)
    }
...
// impl<'a, T> From<ResRef<'a, T>> for ResRef<'a, T>
// where
//     T: Resource + Send + Sync,
// {
...
// /// * `T`: The type of the resource
// /// * `F`: The setup handler (default: `DefaultProvider`)
// pub struct ResMut<'a, T: Resource + Send + Sync> {
//     fetch: ResMut<'a, T>,
// }
...
unsafe impl<'a, T: Resource + Send + Sync> SystemParam for Option<ResMut<'a, T>> {
    type State = ();
...
mod test {
    ...
    fn global_basic() {
        ...
        fn my_system(a: ResRef<UniqueA>, mut b: ResMut<UniqueB>) {
            println!("a = {}", a.0);
            let was = b.0;
            b.0 += 1;
            println!("b = {} -> {}", was, b.0);
    ...
    fn global_optional_basic() {
        ...
        fn my_system(a: Option<ResRef<UniqueA>>, b: Option<ResMut<UniqueB>>) {
            match a {
                None => println!("No A"),
    ...
    fn global_not_present_read() {
        ...
        fn my_system(a: ResRef<UniqueA>, b: Option<ResMut<UniqueB>>) {
            println!("a = {}", a.0);
    ...
    fn global_not_present_write() {
        ...
        fn my_system(a: Option<ResRef<UniqueA>>, mut b: ResMut<UniqueB>) {
            match a {
                None => println!("No A"),
                Some(a) => println!("a = {}", a.0),
            }
    ...
    fn global_read_with_global_read() {
        ...
        fn my_system(a: ResRef<UniqueA>, b: GlobalRef<UniqueA>) {
            println!("a = {}", a.0);
            println!("b = {}", b.0);
        }
    ...
    fn global_write_with_global_write() {
        ...
        fn my_system(a: ResMut<UniqueA>, b: GlobalMut<UniqueA>) {
            println!("a = {}", a.0);
            println!("b = {}", b.0);
        }
    ...
    fn global_write_with_resource_read() {
        ...
        fn my_system(a: ResMut<UniqueA>, b: GlobalRef<UniqueA>) {
            println!("a = {}", a.0);
            println!("b = {}", b.0);
        }
    ...
    fn global_read_with_resource_write() {
        ...
        fn my_system(a: ResRef<UniqueA>, b: GlobalMut<UniqueA>) {
            println!("a = {}", a.0);
            println!("b = {}", b.0);
        }
    ...
    fn unique_double_write() {
        ...
        fn my_system(a: ResMut<UniqueA>, mut b: ResMut<UniqueB>) {
            println!("a = {}", a.0);
            let was = b.0;
            b.0 += 1;
            println!("b = {} -> {}", was, b.0);
    ...
    //     fn my_write_system(a: ResMut<NonSendA>) {
    //         println!("in write system - {:?}", a.0);
    //     }

    //     fn my_read_system(a: ResRef<NonSendA>) {
    //         println!("in read system - {:?}", a.0);
    //     }
