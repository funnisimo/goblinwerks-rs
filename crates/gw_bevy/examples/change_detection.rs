use gw_bevy::{prelude::*, schedule::IntoSystemConfig};
use rand::Rng;
use std::ops::Deref;
...
fn print_changed_entities(
    entities: Entities,
    ages: ReadComp<Age>, // entity_with_added_component: Query<Entity, Added<Age>>,
                         // entity_with_mutated_component: Query<(Entity, &Age), Changed<Age>>,
) {
    for (entity, _) in (&entities, ages.added()).join() {
        println!("    {entity:?} has it's first birthday!");
    }
    for (entity, value) in (&entities, ages.changed()).join() {
        println!("    {entity:?} is now {value:?} frames old");
    }
}

// This system iterates over all entities and increases their age in every frame
fn age_all_entities(mut ages: WriteComp<Age>) {
    for mut age in (&mut ages).join() {
        age.frames += 1;
    }
}

// This system iterates over all entities in every frame and despawns entities older than 2 frames
fn remove_old_entities(mut commands: Commands, entities: Entities, ages: ReadComp<Age>) {
    for (entity, age) in (&entities, &ages).join() {
        if age.frames > 2 {
