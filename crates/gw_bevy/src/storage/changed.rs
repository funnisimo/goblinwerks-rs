...
///
/// WARNING: Do not have a join of only `Changed`s. Otherwise the join will
/// iterate over every single index of the bitset. If you want a join with
/// all `Changed`s, add an `EntitiesRes` to the join as well to bound the
...
/// For usage see [`Join::maybe()`].
///
/// WARNING: Do not have a join of only `ChangedMut`s. Otherwise the join will
/// iterate over every single index of the bitset. If you want a join with
/// all `ChangedMut`s, add an `EntitiesRes` to the join as well to bound the
/// join to all entities that are alive.
///
/// [`Join::maybe()`]: ../join/trait.Join.html#method.maybe
pub struct ChangedMut<J: Join>(pub J);
...
// SAFETY: This is safe as long as `T` implements `ParJoin` safely.  `ChangedMut`
// relies on `T as Join` for all storage access and safely wraps the inner
// `Join` API, so it should also be able to implement `ParJoin`.
#[cfg(feature = "parallel")]
unsafe impl<T> ParJoin for ChangedMut<T> where T: ParJoin {}
