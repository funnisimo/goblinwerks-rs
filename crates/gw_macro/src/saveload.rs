...
use quote::quote;
use syn::{
    parse_quote, Attribute, DataEnum, DataStruct, DeriveInput, Field, GenericParam, Generics,
    Ident, Meta, Type, WhereClause, WherePredicate,
};
...
fn single_parse_outer_from_args(input: syn::parse::ParseStream) -> Result<Attribute, syn::Error> {
    Ok(Attribute {
        ...
        // tokens: input.parse()?,
        meta: Meta::Path(input.call(syn::Path::parse_mod_style)?),
    })
}
